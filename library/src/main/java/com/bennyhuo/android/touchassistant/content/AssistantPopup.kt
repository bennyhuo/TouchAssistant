 ...
 */
class AssistantPopup(private val context: Context, private val mainPageClass: KClass<Page>) : PageManager {

    private var isAdded = false
    private val container by lazy{ contentView.container }
    private val contentView: View by lazy { LayoutInflater.from(context).inflate(R.layout.assistant_popup, null) }
    ...
    private fun initPage(){
        mainPageClass.create(pageContext).apply(this::doShowPage).let(pages::push).onEnter()
    }
    ...
    override fun showPage(clazz: KClass<out Page>) {
        if(currentPage.javaClass.kotlin == clazz) return
        currentPage.onExit()
        pages.firstOrNull { it.javaClass.kotlin == clazz }
            ?: clazz.create(pageContext)
                .apply(this::doShowPage)
    ...
    override fun goBack() {
        pages.peek().takeIf { it::class != mainPageClass }?.apply {
            currentPage.onExit()
    ...
    private fun playShowAnimation() {
        container.addOnLayoutChangeListener(object : View.OnLayoutChangeListener {
            override fun onLayoutChange(v: View, left: Int, top: Int, right: Int, bottom: Int, oldLeft: Int, oldTop: Int, oldRight: Int, oldBottom: Int) {
                ...
                val translationXAnimator = ObjectAnimator.ofFloat(container, "translationX", dx.toFloat(), 0f)
                val translationYAnimator = ObjectAnimator.ofFloat(container, "translationY", dy.toFloat(), 0f)
    ...
    private fun playDismissAnimation() {
        ...
        val translationXAnimator = ObjectAnimator.ofFloat(container, "translationX", 0f, dx.toFloat())
        val translationYAnimator = ObjectAnimator.ofFloat(container, "translationY", 0f, dy.toFloat())
